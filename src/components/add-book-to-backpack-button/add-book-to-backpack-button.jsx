...
const AddBookToBackpackButton = ({
    ...
    const isBookAlreadyLoanForTheUser = () => {
        if (currentUser) {
            const foundBook = currentUserBooks.find(
        ...
        <>
            {isBookAvailableForLoan() ? (
                isBookAlreadyLoanForTheUser() === false ? (
                    book ? (
                        isBookInBackpack(backpackBooks, book._id) ===
                        undefined ? (
                            <Button handleClick={() => dispatch(addBook(book))}>
                                Add to backpack
                            </Button>
                        ) : (
                            <ContentHeadline>
