...
import {
    selectBackpackBooks,
    selectBackpackBooksCount,
} from "../../redux/backpack/backpack.selectors";
import { selectCurrentUser } from "../../redux/user/user.selectors";
import { bookLoanStart } from "../../redux/book/book.actions";
...
const CheckoutPage = ({
    ...
    const handleClick = async () => {
        try {
            if (currentUser) {
                dispatch(bookLoanStart({ books: backpackBooks }));
                await dispatch(clearBackpack());
                history.push(`${match.path}/successful`);
            } else {
                alert("You need to be logged in order to take books.");
                history.push("/sign");
            }
        } catch (err) {
            alert(err.message);
        ...
        <CheckoutContainer>
            ...
            {backpackBooks.length > 0 ? (
                <CheckoutItems>
                    ...
                    <TotalMessage>
                        TOTAL: {backpackBooksCount} book(s)
                    </TotalMessage>
